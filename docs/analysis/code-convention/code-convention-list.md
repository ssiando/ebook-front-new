...
return (
  <>
    <PageTitle title="화면 제목" actionButtonsProps={{ onSearch: handleSearch, onSave: handleSave }}>
      <p>화면 설명 한 줄</p>
    </PageTitle>
...
export default function XxxSearch({ control, onSearch }: XxxSearchProps) {
  ...
  const systemSelectOptions = useMemo(
    () => systemOptionsForAuthUser(user, activeWorkspaceId ?? 0, SYSTEM_OPTIONS), [user, activeWorkspaceId]);
...
export function useXxxQuery(params, searchRevision?, enabled = true) {
  return useQuery({ queryKey: xxxQueryKeys.list({ ...params, searchRevision }), queryFn: () => fetchXxx(params), enabled });
}
```
...
```typescript
export const SOME_OPTIONS = [{ label: 'A', value: 'A' }, { label: 'B', value: 'B' }];
...
const handleSave = useCallback(() => {
  ...
  if (newRows.length === 0) { messageUtil.showAlert('', t('common.msg.no_changes')); return; }
  messageUtil.showConfirm('', t('common.msg.confirm_save'), async () => {
...
const onSubmit = handleSubmit(async (values) => {
  if (isEdit && !isDirty) { messageUtil.showAlert('', t('common.msg.no_changes')); return; }
  // ...
});
```
